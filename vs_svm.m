...
start = 1; % starting bootstrap
N = 200; % # available bootstraps
thr = 95; % SPCA explained variation percentage
frg_thr = zeros(N,1); % # SPCs required
...
for expr = start : N
    ...
    load([fld_gold 'Cpds_Btsp_NoArtm/Cpds_Ref_Bts.mat'])
    load([fld_gold desc '/Frags_Ord_Spr.mat'])
    load([fld_gold desc '/Profs_New.mat'])
    load([fld_gold desc '/Cpds_Prf_New.mat'])
    
    % Removing Artemisinins (cluster of 20 compounds)
    load([fld_gold desc '/Artemisinins.mat'])
    frags_ord(idx_cpds_artm,:) = [];
    profs(idx_cpds_artm,:) = [];
    cpds_prf(idx_cpds_artm,:) = [];
    
    % Note: "prd" variables load/clear at every loop iteration.
    if expr==1
        save([fld_model '/Prd_Bts.mat'],'prd_trn_bts','prd_tst_bts')
    end
    
    % Note: This should be executed ONLY if expr>1
    if expr>1
    ...
    fprintf(' Fragments reduced to %d SPCs.\n',frg_thr_expr);
    fprintf(LOG,' Fragments reduced to %d SPCs.\n',frg_thr_expr);
        
    % Finding all-zero training compounds after SPCA
    idx_zero_trn = find(all(frags_trn==0,2));
    profs_trn(idx_zero_trn,:) = [];
    frags_trn(idx_zero_trn,:) = [];
    idx_trn(idx_zero_trn) = [];
    ...
    % Finding all-zero test compounds after SPCA 
    idx_zero_tst = find(all(frags_tst==0,2));
    profs_tst(idx_zero_tst,:) = [];
    frags_tst(idx_zero_tst,:) = [];
    idx_tst(idx_zero_tst) = [];
    ...
    % shrinking parameter
    param.h = 0;
    % cachesize parameter (default: 100 MB)
    param.m = 15000;
        ...
        for d = 1 : length(param.dset) % length(param.dset) has to be 1.
            for c = 1 : length(param.cset)            
                for g = 1 : length(param.gset)
                    ...
                    % Will be used for "correlation of cosine test similarities (reduced)"
